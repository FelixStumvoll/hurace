...
namespace Hurace.Dal.Test
{
    ...
    public class TestBase
    {
        ...
        protected async Task SetupTimeData()
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                ...
                for (var j = 0; j < 6; j++)
                {
                    ...
                    var raceDataId = await InsertRaceData(raceId.Value, (int) Domain.Enums.RaceDataEvent.SkierSplitTime);
        ...
        {
            var raceId = await SetupRace();
            var raceDataId = await InsertRaceData(raceId.Value, (int) Domain.Enums.RaceDataEvent.RaceStarted);
        ...
        protected async Task<int?> SetupSkierEvent()
        {
            ...
            var raceDataId = await InsertRaceData(raceId.Value, (int) Domain.Enums.RaceDataEvent.SkierStarted);
        ...
        {
            var raceId = await SetupRace();
            return await InsertRaceData(raceId.Value, (int) Domain.Enums.RaceDataEvent.RaceFinished);
        }
